...
def genLabelData(direction):
    Positive_data = pd.read_csv('../data/input_data/pos_sample.csv')

    Negative_data = pd.read_csv('../data/input_data/neg_sample.csv')
    Negative_data = Negative_data[Negative_data['label']==3]

    table = pd.concat([Positive_data,Negative_data])
    table.index = xrange(len(table))
    table = table.drop_duplicates(['C1_start','C2_start','chrom'],keep = False)
    ...
    table['label'][table[table['label']==-1].index]=1
    #table = table[table['label'] != -1] ##可去掉可不去掉？
    if direction != "imb":
        table = balance_data_dist(table)
    else:
        table = table

    table.to_csv('../data/input_data/LabelData_neg3_balance.csv',index=False)
    
    print 'LabelData : %d' %(len(table))
    print "Positive : %d Negative : %d\n" %(len(table[table['label'] == 1]),len(table[table['label'] != 1]))


if __name__ == '__main__':
    direction = 'noimb'
    genLabelData(direction)